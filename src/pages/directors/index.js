...
export default function DirectorsPage({ directors, movies }) {
  return (
    <div className="container mx-auto py-10">
      <h2 className="text-3xl font-semibold mb-4">Directors</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {directors.map((director) => {
          const directedMovies = movies.filter((movie) => movie.directorId === director.id);

          return (
            <div key={director.id} style={{borderRadius: '0.5rem', boxShadow: '0 1px 3px 0 rgba(0,0,0,.1), 0 1px 2px -1px rgba(0,0,0,.1)', padding: '1rem'}}>
            <Link key={director.id} href={`/directors/${director.id}`}>
              ...
              <div>
                ...
                <h3 className="text-lg font-semibold mt-2">Movies Directed:</h3>
